...
function reachInteract(scene: Scene, layout: any, it: any): boolean {
  ...
  // early-exit target check inside loop
...
async function main() {
  ...
  for (let seed = 1; seed <= N; seed++) {
    for (let floor = 1; floor <= 3; floor++) {
      ...
      if (stairs && !reachInteract(scene, layout, stairs)) fails.push(`seed=${seed} f=${floor} STAIRS`);
      if (extract && !reachInteract(scene, layout, extract)) fails.push(`seed=${seed} f=${floor} EXTRACT`);
    }
  }
  console.log(`FINE 8-dir sweep: ${checked} floors, ${fails.length} failures`);
